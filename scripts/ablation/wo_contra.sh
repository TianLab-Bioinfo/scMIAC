#!/bin/bash
# w/o Contrastive Learning 消融实验
# 使用 MSE 损失代替对比学习损失
#
# Usage:
#   bash wo_contra.sh                              # 运行所有数据集
#   bash wo_contra.sh 10x share                    # 运行指定数据集
#   bash wo_contra.sh 10x --num-epochs 5           # 指定数据集 + 自定义参数
#   bash wo_contra.sh --num-epochs 5 --print-step 1 # 所有数据集 + 自定义参数
...
echo "======================================"
echo "Running w/o Contrastive ablation"
echo "Datasets: ${DATASETS[*]}"
echo "Using default configuration from scmiac package"
echo "Modification: --anchor-loss-type mse"
...
for dataset in ${DATASETS[@]}; do
    output_dir="${PROJECT_ROOT}/data/${dataset}/output/ablation/wo_contra"
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo ">>> ERROR: Failed to run wo_contra on dataset ${dataset} (exit code: $EXIT_CODE)" >&2
    ...
    # 覆盖已有的 wo_contra 记录（如果存在）
    if grep -q "^wo_contra," "$time_file" 2>/dev/null; then
        grep -v "^wo_contra," "$time_file" > "${time_file}.tmp"
        mv "${time_file}.tmp" "$time_file"
    fi
    
    # 追加新的时间记录
    echo "wo_contra,${DATASET_ELAPSED},${DATASET_MINUTES}m ${DATASET_SECONDS}s,$(date '+%Y-%m-%d %H:%M:%S')" >> "$time_file"
    ...
    # 覆盖已有的 wo_contra 记录（如果存在）
    if grep -q "^wo_contra," "$memory_file" 2>/dev/null; then
        grep -v "^wo_contra," "$memory_file" > "${memory_file}.tmp"
        mv "${memory_file}.tmp" "$memory_file"
    fi
    
    # 追加新的内存记录
    echo "wo_contra,${PEAK_MEMORY_KB},${PEAK_MEMORY_MB},${PEAK_MEMORY_GB},$(date '+%Y-%m-%d %H:%M:%S')" >> "$memory_file"
    echo ">>> Memory saved to: $memory_file"
done

echo ""
echo "======================================"
echo "w/o Contrastive ablation completed!"
