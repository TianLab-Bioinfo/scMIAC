#!/bin/bash
# w/o Balanced Sampler 消融实验
# 禁用平衡采样器，使用普通随机采样
#
# Usage:
#   bash wo_bs.sh                              # 运行所有数据集
#   bash wo_bs.sh 10x share                    # 运行指定数据集
#   bash wo_bs.sh 10x --num-epochs 5           # 指定数据集 + 自定义参数
#   bash wo_bs.sh --num-epochs 5 --print-step 1 # 所有数据集 + 自定义参数
...
echo "======================================"
echo "Running w/o Balanced Sampler ablation"
echo "Datasets: ${DATASETS[*]}"
echo "Using default configuration from scmiac package"
echo "Modification: --no-balanced-sampler"
...
for dataset in ${DATASETS[@]}; do
    output_dir="${PROJECT_ROOT}/data/${dataset}/output/ablation/wo_bs"
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo ">>> ERROR: Failed to run wo_bs on dataset ${dataset} (exit code: $EXIT_CODE)" >&2
    ...
    if grep -q "^wo_bs," "$time_file" 2>/dev/null; then
        grep -v "^wo_bs," "$time_file" > "${time_file}.tmp"
        mv "${time_file}.tmp" "$time_file"
    fi
    echo "wo_bs,${DATASET_ELAPSED},${DATASET_MINUTES}m ${DATASET_SECONDS}s,$(date '+%Y-%m-%d %H:%M:%S')" >> "$time_file"
    ...
    if grep -q "^wo_bs," "$memory_file" 2>/dev/null; then
        grep -v "^wo_bs," "$memory_file" > "${memory_file}.tmp"
        mv "${memory_file}.tmp" "$memory_file"
    fi
    echo "wo_bs,${PEAK_MEMORY_KB},${PEAK_MEMORY_MB},${PEAK_MEMORY_GB},$(date '+%Y-%m-%d %H:%M:%S')" >> "$memory_file"
    echo ">>> Memory saved to: $memory_file"
done

echo ""
echo "======================================"
echo "w/o Balanced Sampler ablation completed!"
