#!/bin/bash
# w/o VAE 消融实验
# 禁用 VAE 重构和 KL 散度损失，仅保留锚点对齐损失
#
# Usage:
#   bash wo_vae.sh                              # 运行所有数据集
#   bash wo_vae.sh 10x share                    # 运行指定数据集
#   bash wo_vae.sh 10x --num-epochs 5           # 指定数据集 + 自定义参数
#   bash wo_vae.sh --num-epochs 5 --print-step 1 # 所有数据集 + 自定义参数
...
echo "======================================"
echo "Running w/o VAE ablation"
echo "Datasets: ${DATASETS[*]}"
echo "Using default configuration from scmiac package"
echo "Modification: Set all VAE loss weights to 0"
...
for dataset in ${DATASETS[@]}; do
    output_dir="${PROJECT_ROOT}/data/${dataset}/output/ablation/wo_vae"
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo ">>> ERROR: Failed to run wo_vae on dataset ${dataset} (exit code: $EXIT_CODE)" >&2
    ...
    if grep -q "^wo_vae," "$time_file" 2>/dev/null; then
        grep -v "^wo_vae," "$time_file" > "${time_file}.tmp"
        mv "${time_file}.tmp" "$time_file"
    fi
    echo "wo_vae,${DATASET_ELAPSED},${DATASET_MINUTES}m ${DATASET_SECONDS}s,$(date '+%Y-%m-%d %H:%M:%S')" >> "$time_file"
    ...
    if grep -q "^wo_vae," "$memory_file" 2>/dev/null; then
        grep -v "^wo_vae," "$memory_file" > "${memory_file}.tmp"
        mv "${memory_file}.tmp" "$memory_file"
    fi
    echo "wo_vae,${PEAK_MEMORY_KB},${PEAK_MEMORY_MB},${PEAK_MEMORY_GB},$(date '+%Y-%m-%d %H:%M:%S')" >> "$memory_file"
    echo ">>> Memory saved to: $memory_file"
done

echo ""
echo "======================================"
echo "w/o VAE ablation completed!"
