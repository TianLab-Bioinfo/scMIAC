#!/bin/bash
# w/o MNN 消融实验
# 使用随机锚点采样代替互近邻算法（仍保留细胞类型过滤）
#
# Usage:
#   bash wo_mnn.sh                              # 运行所有数据集
#   bash wo_mnn.sh 10x share                    # 运行指定数据集
#   bash wo_mnn.sh 10x --num-epochs 5           # 指定数据集 + 自定义参数
#   bash wo_mnn.sh --num-epochs 5 --print-step 1 # 所有数据集 + 自定义参数
...
echo "======================================"
echo "Running w/o MNN ablation"
echo "Datasets: ${DATASETS[*]}"
echo "Using default configuration from scmiac package"
echo "Modification: --anchor-generation random --random-anchor-seed 42"
...
for dataset in ${DATASETS[@]}; do
    output_dir="${PROJECT_ROOT}/data/${dataset}/output/ablation/wo_mnn"
    ...
    /usr/bin/time -v python "$IMPL_SCRIPT" \
        ...
        --anchor-generation random \
        --random-anchor-seed 42 \
        --identifier "wo_mnn" \
        --gpu-csv "${GPU_CSV}" \
        "${EXTRA_ARGS[@]}" \
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo ">>> ERROR: Failed to run wo_mnn on dataset ${dataset} (exit code: $EXIT_CODE)" >&2
    ...
    if grep -q "^wo_mnn," "$time_file" 2>/dev/null; then
        grep -v "^wo_mnn," "$time_file" > "${time_file}.tmp"
        mv "${time_file}.tmp" "$time_file"
    fi
    echo "wo_mnn,${DATASET_ELAPSED},${DATASET_MINUTES}m ${DATASET_SECONDS}s,$(date '+%Y-%m-%d %H:%M:%S')" >> "$time_file"
    ...
    if grep -q "^wo_mnn," "$memory_file" 2>/dev/null; then
        grep -v "^wo_mnn," "$memory_file" > "${memory_file}.tmp"
        mv "${memory_file}.tmp" "$memory_file"
    fi
    echo "wo_mnn,${PEAK_MEMORY_KB},${PEAK_MEMORY_MB},${PEAK_MEMORY_GB},$(date '+%Y-%m-%d %H:%M:%S')" >> "$memory_file"
    echo ">>> Memory saved to: $memory_file"
done

echo ""
echo "======================================"
echo "w/o MNN ablation completed!"
