...
class SCOTv2(object):
	...
	def __init__(self, data, device='cpu'):
		...
		print(f"SCOTv2 using device: {self.device}")
	...
	def _init_marginals(self):
		...
		for i in range(len(self.data)):
			...
			marginalDist=torch.ones(num_cells, device=self.device)/num_cells
	...
	def construct_graph(self, k=20, mode= "connectivity", metric="cosine"):
		# NOTE: 原始SCOTv2默认使用 metric="correlation", 但在 numpy 2.x 环境下
		# sklearn.neighbors.kneighbors_graph 使用 correlation 会导致段错误(signal 11)
		# 改用 metric="cosine", 两者在数学上非常相似（cosine相似度 = 1 - Pearson相关系数），
		# 不影响算法效果，且 cosine 计算性能更好（32k样本: cosine 13s vs correlation 卡死）
	...
	def exp_unbalanced_gw(self,a, dx, b, dy, eps=0.01, rho=1.0, rho2=None, nits_plan=3000, tol_plan=1e-6, nits_sinkhorn=3000, tol_sinkhorn=1e-6):
		...
		pi_prev = torch.zeros_like(pi, device=a.device)
		up, vp = None, None
	...
	def find_correspondences(self, normalize=True, norm="l2", bySample=True, k=20, mode= "connectivity", metric="cosine",  eps=0.01, rho=1.0, rho2=None):
		# NOTE: metric 默认改为 "cosine" 而非原始的 "correlation"，原因见 construct_graph() 注释
		# Normalize 
		if normalize:
		...
		for i in range(len(self.data)-1):
			...
			a,b =torch.Tensor(self.marginals[0]).to(self.device), torch.Tensor(self.marginals[i+1]).to(self.device)
			dx, dy= torch.Tensor(self.graphDists[0]).to(self.device), torch.Tensor(self.graphDists[i+1]).to(self.device)
	...
	def barycentric_projection(self):
		...
		for i in range(0,len(self.couplings)):
			if self.device.type == 'cuda':
				# GPU模式：使用PyTorch计算
				coupling = self.couplings[i].T  # PyTorch转置
				weights = torch.sum(coupling, dim=1, keepdim=True)
				data0_tensor = torch.tensor(self.data[0], device=self.device, dtype=torch.float32)
				projected_data = torch.matmul(coupling / weights, data0_tensor)
				aligned_datasets.append(projected_data.cpu().numpy())
			else:
				# CPU模式：保持原有NumPy实现
				coupling=np.transpose(self.couplings[i].cpu().numpy())
	...
	def coembed_datasets(self, Lambda=1.0, out_dim=10):
		...
		"""
		if self.device.type == 'cuda':
			return self._coembed_datasets_gpu(Lambda, out_dim)
		else:
			return self._coembed_datasets_cpu(Lambda, out_dim)
	
	def _coembed_datasets_cpu(self, Lambda=1.0, out_dim=10):
		"""原有的NumPy CPU实现"""
		n_datasets = len(self.data)
		H0 = []
		L = []
		for i in range(n_datasets-1):
			self.couplings[i] = self.couplings[i].cpu().numpy()*np.shape(self.data[i])[0]
	...
	def _coembed_datasets_gpu(self, Lambda=1.0, out_dim=10):
		"""新增的PyTorch GPU实现"""
		n_datasets = len(self.data)
		L = []
		...
		couplings_scaled = []
		for i in range(n_datasets-1):
			coupling_scaled = self.couplings[i] * self.data[i].shape[0]
			couplings_scaled.append(coupling_scaled)
		...
		for i in range(n_datasets):
			...
			W[index_pos] = 1/W[index_pos]
			D = np.diag(np.dot(W, np.ones(np.shape(W)[1])))
			L.append(torch.tensor(D - W, device=self.device, dtype=torch.float32))
		
		# 计算Sigma矩阵
		Sigma_x = []
		Sigma_y = []
		for i in range(n_datasets-1):
			coupling = couplings_scaled[i]
			# Sigma_y: 列和的对角矩阵
			sigma_y_diag = torch.sum(coupling, dim=0)
			Sigma_y.append(torch.diag(sigma_y_diag))
			# Sigma_x: 行和的对角矩阵
			sigma_x_diag = torch.sum(coupling, dim=1)
			Sigma_x.append(torch.diag(sigma_x_diag))
		
		# 构建S_xy, S_xx, S_yy
		S_xy = couplings_scaled[0]
		S_xx = L[0] + Lambda * Sigma_x[0]
		S_yy = L[-1] + Lambda * Sigma_y[0]
		
		for i in range(1, n_datasets-1):
			S_xy = torch.vstack((S_xy, couplings_scaled[i]))
			# 构建块对角矩阵
			S_xx = torch.block_diag(S_xx, L[i] + Lambda * Sigma_x[i])
			S_yy = S_yy + Lambda * Sigma_y[i]
		
		# 计算H_x: S_xx^{-1/2}
		eigenvalues, eigenvectors = torch.linalg.eigh(S_xx)
		eigenvalues = eigenvalues + 1e-12
		eigenvalues_sqrt_inv = torch.diag(eigenvalues ** (-0.5))
		H_x = torch.mm(torch.mm(eigenvectors, eigenvalues_sqrt_inv), eigenvectors.T)
		
		# 计算H_y: S_yy^{-1/2}
		eigenvalues, eigenvectors = torch.linalg.eigh(S_yy)
		eigenvalues = eigenvalues + 1e-12
		eigenvalues_sqrt_inv = torch.diag(eigenvalues ** (-0.5))
		H_y = torch.mm(torch.mm(eigenvectors, eigenvalues_sqrt_inv), eigenvectors.T)
		
		# 计算H = H_x @ S_xy @ H_y
		H = torch.mm(torch.mm(H_x, S_xy), H_y)
		
		# SVD分解
		U, sigma, Vt = torch.linalg.svd(H, full_matrices=False)
		V = Vt.T
		...
		integrated_data = []
		for i in range(n_datasets-1):
			integrated_data.append(fx[num[i]:num[i+1]].cpu().numpy())
		integrated_data.append(fy.cpu().numpy())
		
		return integrated_data

	def align(self,normalize=True, norm="l2", bySample=True, k=20, mode= "connectivity", metric="cosine",  eps=0.01, rho=1.0, rho2=None, projMethod="embedding", Lambda=1.0, out_dim=10):
		# NOTE: metric 默认改为 "cosine" 而非原始的 "correlation"，原因见 construct_graph() 注释
