#!/bin/bash
# MultiVI 方法批量运行脚本 - 垂直整合测评
# 在配对数据集上测试不同配对比例的性能
#
# Usage:
#   bash run_multivi.sh                             # 运行所有配对数据集和所有配对比例
#   bash run_multivi.sh 10x                         # 运行指定数据集的所有配对比例
#   bash run_multivi.sh --device cpu 10x            # 使用 CPU
#   bash run_multivi.sh --help                      # 显示帮助
...
IMPL_SCRIPT="${SCRIPT_DIR}/../implementations/multivi/main.py"
...
Usage:
  bash run_multivi.sh [OPTIONS] [<dataset1> ...]

Options:
  --device <device>           设备选择 (default: cuda:0, 可选: cuda:1, cpu)
  --max-epochs <num>          训练轮数 (default: 使用MultiVI官方默认值 500)
  --n-top-genes <num>         每个模态的高变特征数 (default: 4000)
...
Examples:
  bash run_multivi.sh                              # 运行所有配对数据集和所有配对比例
  bash run_multivi.sh 10x                          # 运行 10x 数据集的所有配对比例
  bash run_multivi.sh 10x share                    # 运行 10x 和 share 的所有配对比例
  bash run_multivi.sh --device cpu 10x             # 使用 CPU 运行 10x
  bash run_multivi.sh --max-epochs 400 10x         # 自定义训练轮数

Output:
  Results will be saved to data/<dataset>/output/vertical_methods/multivi/<ratio>/
...
echo "======================================"
echo "Running MultiVI (Vertical Integration)"
echo "Datasets: ${DATASETS[*]}"
echo "Paired ratios: ${PAIRED_RATIOS[*]}"
echo "Device: $DEVICE"
echo "Max epochs: ${MAX_EPOCHS:-default (500)}"
...
for dataset in "${DATASETS[@]}"; do
    ...
    for ratio in "${PAIRED_RATIOS[@]}"; do
        output_dir="${data_root}/${dataset}/output/vertical_methods/multivi/${ratio}"
        paired_cells_file="${data_root}/${dataset}/input/paired_cells/paired_${ratio}_cells.txt"
        
        # GPU 统计文件路径（按 ratio 分文件）
        GPU_CSV_PATH="${data_root}/${dataset}/output/vertical_methods/gpu_${ratio}.csv"
        ...
        # 只在明确指定时才添加 max-epochs 参数
        if [ -n "$MAX_EPOCHS" ]; then
            CMD_ARGS+=(--max-epochs "$MAX_EPOCHS")
        fi
        
        /usr/bin/time -v python "$IMPL_SCRIPT" "${CMD_ARGS[@]}" \
            2> "$TIME_LOG" || EXIT_CODE=$?
        
        # 检查执行是否成功
        if [ $EXIT_CODE -ne 0 ]; then
            echo ">>> ERROR: Failed to run MultiVI on dataset=$dataset, ratio=$ratio (exit code: $EXIT_CODE)" >&2
        ...
        # 覆盖已有的记录
        if grep -q "^multivi,${ratio}," "$time_file" 2>/dev/null; then
            grep -v "^multivi,${ratio}," "$time_file" > "${time_file}.tmp"
            mv "${time_file}.tmp" "$time_file"
        fi
        
        # 追加新的时间记录
        echo "multivi,${ratio},${RUN_ELAPSED},${RUN_MINUTES}m ${RUN_SECONDS}s,$(date '+%Y-%m-%d %H:%M:%S')" >> "$time_file"
        ...
        # 覆盖已有的记录
        if grep -q "^multivi,${ratio}," "$memory_file" 2>/dev/null; then
            grep -v "^multivi,${ratio}," "$memory_file" > "${memory_file}.tmp"
            mv "${memory_file}.tmp" "$memory_file"
        fi
        
        # 追加新的内存记录
        echo "multivi,${ratio},${PEAK_MEMORY_KB},${PEAK_MEMORY_MB},${PEAK_MEMORY_GB},$(date '+%Y-%m-%d %H:%M:%S')" >> "$memory_file"
