#!/bin/bash
# UniPort 方法批量运行脚本 - 对角线整合测评
# 对角线整合测评 - 循环运行所有数据集进行基准测试
#
# Usage:
#   bash run_uniport.sh                    # 运行所有数据集（使用细胞类型先验）
#   bash run_uniport.sh 10x share          # 运行指定数据集
#   bash run_uniport.sh --no-prior 10x     # 禁用细胞类型先验
#   bash run_uniport.sh --help             # 显示帮助
...
IMPL_SCRIPT="${SCRIPT_DIR}/../implementations/uniport/main.py"
...
Usage:
  bash run_uniport.sh [datasets...] [extra_args...]
  
  datasets     指定要运行的数据集（可选，默认运行所有）
  extra_args   传递给 uniport 脚本的额外参数

Available datasets: ${ALL_DATASETS[*]}

Examples:
  bash run_uniport.sh                                      # 运行所有数据集（默认使用先验）
  bash run_uniport.sh 10x share                            # 只运行 10x 和 share
  bash run_uniport.sh --no-prior                           # 禁用先验运行所有数据集
  bash run_uniport.sh --no-prior 10x                       # 禁用先验运行 10x
  bash run_uniport.sh --rna-celltype-key custom_rna 10x    # 自定义RNA列名
  bash run_uniport.sh --atac-celltype-key custom_atac 10x  # 自定义ATAC列名
  bash run_uniport.sh 10x --alpha 3.0                      # 自定义alpha参数

Common Options:
  --no-prior                     禁用细胞类型先验信息（默认使用先验）
  --rna-celltype-key <key>       RNA细胞类型列名（默认: cell_type）
  --atac-celltype-key <key>      ATAC细胞类型列名（默认: pred）
  --alpha <value>                先验矩阵alpha参数（默认: 2.0）
  --device <device>              计算设备（默认: cuda:0）
  --seed <seed>                  随机种子（默认: 24）

Output:
  With prior (default): data/<dataset>/output/diagnal_methods/uniport/
  Without prior (--no-prior): data/<dataset>/output/diagnal_methods/uniport_noct/
...
echo "======================================"
echo "Running UniPort on datasets: ${DATASETS[*]}"
...
for dataset in "${DATASETS[@]}"; do
    ...
    if [[ " ${EXTRA_ARGS[*]} " =~ " --no-prior " ]]; then
        output_dir="${data_root}/${dataset}/output/diagnal_methods/uniport_noct"
    else
        output_dir="${data_root}/${dataset}/output/diagnal_methods/uniport"
    fi
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo ">>> ERROR: Failed to run uniport on dataset $dataset (exit code: $EXIT_CODE)" >&2
