#!/bin/bash
# Seurat3 方法批量运行脚本 - 对角线整合测评
# 对角线整合测评 - 循环运行所有数据集进行基准测试
#
# Usage:
#   bash run_seurat3.sh                    # 运行所有数据集
#   bash run_seurat3.sh 10x share          # 运行指定数据集
#   bash run_seurat3.sh --help             # 显示帮助
...
IMPL_SCRIPT="${SCRIPT_DIR}/../implementations/seurat3/main.R"
...
Usage:
  bash run_seurat3.sh [<dataset1> ...]  # 运行指定数据集
  bash run_seurat3.sh --help            # 显示帮助
...
Examples:
  bash run_seurat3.sh               # 运行所有数据集
  bash run_seurat3.sh 10x share     # 只运行 10x 和 share

Output:
  data/<dataset>/output/diagnal_methods/seurat3/
...
echo "======================================"
echo "Running Seurat3 on datasets: ${DATASETS[*]}"
echo "======================================"
...
for dataset in "${DATASETS[@]}"; do
    ...
    output_dir="${data_root}/${dataset}/output/diagnal_methods/seurat3"
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo ">>> ERROR: Failed to run seurat3 on dataset $dataset (exit code: $EXIT_CODE)" >&2
    ...
    # 覆盖已有的该方法记录（如果存在）
    if grep -q "^seurat3," "$time_file" 2>/dev/null; then
        grep -v "^seurat3," "$time_file" > "${time_file}.tmp"
        mv "${time_file}.tmp" "$time_file"
    fi
    
    # 追加新的时间记录
    echo "seurat3,${DATASET_ELAPSED},${DATASET_MINUTES}m ${DATASET_SECONDS}s,$(date '+%Y-%m-%d %H:%M:%S')" >> "$time_file"
    ...
    # 覆盖已有的该方法记录（如果存在）
    if grep -q "^seurat3," "$memory_file" 2>/dev/null; then
        grep -v "^seurat3," "$memory_file" > "${memory_file}.tmp"
        mv "${memory_file}.tmp" "$memory_file"
    fi
    
    # 追加新的内存记录
    echo "seurat3,${PEAK_MEMORY_KB},${PEAK_MEMORY_MB},${PEAK_MEMORY_GB},$(date '+%Y-%m-%d %H:%M:%S')" >> "$memory_file"
