#!/bin/bash
# scMIAC 方法批量运行脚本 - 对角线整合测评
# 在所有数据集上进行基准测试
#
# Usage:
#   bash run_scmiac.sh                    # 运行所有数据集
#   bash run_scmiac.sh 10x share          # 运行指定数据集
...
Usage:
  ...
  extra_args   传递给 scmiac CLI 的额外参数（如 --num-epochs, --device 等）

Available datasets: ${ALL_DATASETS[*]}

Examples:
  bash run_scmiac.sh                                  # 运行所有数据集（默认参数）
  bash run_scmiac.sh 10x share                        # 运行 10x 和 share
  bash run_scmiac.sh --num-epochs 50 --print-step 5   # 所有数据集，自定义参数
  bash run_scmiac.sh 10x --num-epochs 50              # 10x 数据集，50个epochs
  bash run_scmiac.sh 10x share --device cuda:1        # 多个数据集 + 自定义设备
...
Output:
  Results will be saved to data/<dataset>/output/diagnal_methods/scmiac/
...
echo "======================================"
echo "Running scMIAC (Diagonal Integration)"
...
for dataset in "${DATASETS[@]}"; do
    output_dir="${PROJECT_ROOT}/data/${dataset}/output/diagnal_methods/scmiac"
    ...
    # GPU统计文件保存路径（与time.csv、memory.csv同级）
    GPU_CSV_PATH="${PROJECT_ROOT}/data/${dataset}/output/diagnal_methods/gpu.csv"
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo ">>> ERROR: Failed to run scmiac on dataset $dataset (exit code: $EXIT_CODE)" >&2
    ...
    # 保存时间信息到 CSV
    time_dir="${PROJECT_ROOT}/data/${dataset}/output/diagnal_methods"
    ...
    # 覆盖已有的 scmiac 记录（如果存在）
    if grep -q "^scmiac," "$time_file" 2>/dev/null; then
        # 使用临时文件过滤掉旧记录
        grep -v "^scmiac," "$time_file" > "${time_file}.tmp"
        mv "${time_file}.tmp" "$time_file"
    fi
    
    # 追加新的时间记录
    echo "scmiac,${DATASET_ELAPSED},${DATASET_MINUTES}m ${DATASET_SECONDS}s,$(date '+%Y-%m-%d %H:%M:%S')" >> "$time_file"
    ...
    # 覆盖已有的 scmiac 记录（如果存在）
    if grep -q "^scmiac," "$memory_file" 2>/dev/null; then
        # 使用临时文件过滤掉旧记录
        grep -v "^scmiac," "$memory_file" > "${memory_file}.tmp"
        mv "${memory_file}.tmp" "$memory_file"
    fi
    
    # 追加新的内存记录
    echo "scmiac,${PEAK_MEMORY_KB},${PEAK_MEMORY_MB},${PEAK_MEMORY_GB},$(date '+%Y-%m-%d %H:%M:%S')" >> "$memory_file"
