#!/bin/bash
# SCOTv2 方法批量运行脚本 - 对角线整合测评
# 对角线整合测评 - 循环运行所有数据集进行基准测试
#
# Usage:
#   bash run_scotv2.sh                    # 运行所有数据集（CPU）
#   bash run_scotv2.sh 10x share          # 运行指定数据集（CPU）
#   bash run_scotv2.sh --device cuda      # 在 GPU 上运行所有数据集
#   bash run_scotv2.sh --device cuda 10x  # 在 GPU 上运行指定数据集
#   bash run_scotv2.sh --help             # 显示帮助
...
IMPL_SCRIPT="${SCRIPT_DIR}/../implementations/scot/main.py"
...
Usage:
  bash run_scotv2.sh [OPTIONS] [<dataset1> ...]

Options:
  --device <cpu|cuda>   指定计算设备（默认: cpu）
  --help, -h            显示帮助信息

Available datasets: ${ALL_DATASETS[*]}

Examples:
  bash run_scotv2.sh                     # 在 CPU 上运行所有数据集
  bash run_scotv2.sh 10x share           # 在 CPU 上运行 10x 和 share
  bash run_scotv2.sh --device cuda       # 在 GPU 上运行所有数据集
  bash run_scotv2.sh --device cuda 10x   # 在 GPU 上运行 10x 数据集

Output:
  Results will be saved to data/<dataset>/output/diagnal_methods/scotv2/

Note:
  - SCOTv2 是无监督方法，不使用细胞类型先验信息
  - 使用 GPU 需要 CUDA 可用且 PyTorch 支持 CUDA
  - 默认使用 CPU，向后兼容
...
echo "======================================"
echo "Running SCOTv2 on datasets: ${DATASETS[*]}"
echo "Device: ${DEVICE}"
echo "Mode: Unsupervised (no prior information)"
echo "======================================"
...
for dataset in "${DATASETS[@]}"; do
    ...
    output_dir="${data_root}/${dataset}/output/diagnal_methods/scotv2"
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo ">>> ERROR: Failed to run scotv2 on dataset $dataset (exit code: $EXIT_CODE)" >&2
    ...
    # 覆盖已有的 scotv2 记录（如果存在）
    if grep -q "^scotv2," "$time_file" 2>/dev/null; then
        # 使用临时文件过滤掉旧记录
        grep -v "^scotv2," "$time_file" > "${time_file}.tmp"
        mv "${time_file}.tmp" "$time_file"
    fi
    
    # 追加新的时间记录
    echo "scotv2,${DATASET_ELAPSED},${DATASET_MINUTES}m ${DATASET_SECONDS}s,$(date '+%Y-%m-%d %H:%M:%S')" >> "$time_file"
    ...
    # 覆盖已有的 scotv2 记录（如果存在）
    if grep -q "^scotv2," "$memory_file" 2>/dev/null; then
        # 使用临时文件过滤掉旧记录
        grep -v "^scotv2," "$memory_file" > "${memory_file}.tmp"
        mv "${memory_file}.tmp" "$memory_file"
    fi
    
    # 追加新的内存记录
    echo "scotv2,${PEAK_MEMORY_KB},${PEAK_MEMORY_MB},${PEAK_MEMORY_GB},$(date '+%Y-%m-%d %H:%M:%S')" >> "$memory_file"
